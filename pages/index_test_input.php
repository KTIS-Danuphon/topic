...
<body data-pc-preset="preset-1" data-pc-direction="ltr" data-pc-theme="light">
  ...
  <div class="modal fade" id="new-post-modal" data-bs-keyboard="false" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
            ...
            <div class="form-group row" style="padding: 0px 5%">
              ...
              <div class="col-md-12">
                ...
                <div class="form-group row align-items-center">
                  <label for="post_add_file1">แนบไฟล์ที่ 1</label>
                  <div class="col-md-10">
                    <input type="file" name="post_add_file1" class="form-control" id="post_add_file1"
                      onchange="checkFileSize(this)">
                    <div class="invalid-feedback">ไฟล์ต้องมีขนาดไม่เกิน 2 MB</div>
                  </div>
                  <div class="col-md-2">
                    <i class="bi bi-backspace text-danger" style="cursor:pointer"
                      role="button" aria-label="ล้างไฟล์"
                      onclick="clearFileInput(1)"> ล้างไฟล์</i>
                  </div>
                </div>

                <!-- file2 -->
                <div class="form-group row align-items-center">
                  <label for="post_add_file2">แนบไฟล์ที่ 2</label>
                  <div class="col-md-10">
                    <input type="file" name="post_add_file2" class="form-control" id="post_add_file2"
                      onchange="checkFileSize(this)">
                    <div class="invalid-feedback">ไฟล์ต้องมีขนาดไม่เกิน 2 MB</div>
                  </div>
                  <div class="col-md-2">
                    <i class="bi bi-backspace text-danger" style="cursor:pointer"
                      role="button" aria-label="ล้างไฟล์"
                      onclick="clearFileInput(2)"> ล้างไฟล์</i>
                  </div>
                </div>

                <!-- file3 -->
                <div class="form-group row align-items-center">
                  <label for="post_add_file3">แนบไฟล์ที่ 3</label>
                  <div class="col-md-10">
                    <input type="file" name="post_add_file3" class="form-control" id="post_add_file3"
                      onchange="checkFileSize(this)">
                    <div class="invalid-feedback">ไฟล์ต้องมีขนาดไม่เกิน 2 MB</div>
                  </div>
                  <div class="col-md-2">
                    <i class="bi bi-backspace text-danger" style="cursor:pointer"
                      role="button" aria-label="ล้างไฟล์"
                      onclick="clearFileInput(3)"> ล้างไฟล์</i>
                  </div>
                </div>

                <!-- file4 -->
                <div class="form-group row align-items-center">
                  <label for="post_add_file4">แนบไฟล์ที่ 4</label>
                  <div class="col-md-10">
                    <input type="file" name="post_add_file4" class="form-control" id="post_add_file4"
                      onchange="checkFileSize(this)">
                    <div class="invalid-feedback">ไฟล์ต้องมีขนาดไม่เกิน 2 MB</div>
                  </div>
                  <div class="col-md-2">
                    <i class="bi bi-backspace text-danger" style="cursor:pointer"
                      role="button" aria-label="ล้างไฟล์"
                      onclick="clearFileInput(4)"> ล้างไฟล์</i>
                  </div>
                </div>

                <script>
                  function clearFileInput(id) {
                    const input = document.getElementById('post_add_file' + id);
                    input.value = '';
                    input.classList.remove('is-invalid'); // เอา error ออกด้วยเวลาเคลียร์ไฟล์
                  }
  ...
  <script>
    document.getElementById('mentionForm').addEventListener('submit', async function(e) {
      ...
      if (text === '') {
        ...
        // console.log("formDataV:", formData);
        const form = document.getElementById("mentionForm");
        const hiddenInput = document.getElementById("post_content");
        const content = editor.innerHTML;
        ...
        console.log("farm Data:", formData.get("post_content"));

        try {
          const response = await fetch('action_add_post2.php', {
              method: 'POST',
              body: formData
    ...
    const editors_input = document.getElementById("editor");
    const editors_edit_input = document.getElementById("editor_edit");

    editors_input.addEventListener("input", () => {
      editors_input.querySelectorAll("span.mention").forEach(span => { //เพิ่มช่ิงว่าง
        const next = span.nextSibling;

        // ถ้า next node ไม่ใช่ text node หรือ text node ไม่มีช่องว่างต่อท้าย
    ...
    editors_edit_input.addEventListener("input", () => {
      editors_edit_input.querySelectorAll("span.mention").forEach(span => { //เพิ่มช่ิงว่าง
        const next = span.nextSibling;
        ...
        if (data.success) {
          ...
          div.querySelectorAll("a").forEach(a => {
            ...
            if (span) {
              ...
              // ตรวจสอบว่าหลัง <a> มี node อะไรอยู่แล้วไหม
              const next = a.nextSibling;
              if (!next || next.nodeType !== Node.TEXT_NODE || !next.textContent.trim()) {
                // ถ้าไม่มี text node หรือว่างเกินไป → ใส่ space ให้
                const space = document.createTextNode('\u00A0'); // &nbsp;
                a.replaceWith(newSpan, space);
              } else {
                // ถ้ามี text อยู่แล้ว → ไม่ต้องใส่ space
